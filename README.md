...
> 适用场景：MaaS 平台导出的 CSV 格式请求日志，需先转换为 JSONL 再运行分析。  
> **每份 CSV 对应一个模型，必须分开分析**——不同模型的 KV cache 不可共享（参见 CLAUDE.md §5a 单模型假设）。
...
| 列索引 | 字段含义 | JSONL 目标字段 |
|---|---|---|
| 0 | 用户请求 ID | `request_id` |
| 1 | 租户 ID | `user_id` |
| 2 | 请求参数（JSON 字符串，含 `messages[]`）| 提取为 `raw_prompt` |
| 3 | 时间戳（Unix 浮点或 ISO-8601）| `timestamp` |

`request_params` 中 `messages[]` 字段格式（标准 OpenAI Chat API）：
```json
{
  "messages": [
...
转换后 `raw_prompt` = `"system: You are a helpful assistant.\n\nuser: What is the capital of France?"`
...
python scripts/convert_csv_to_jsonl.py \
    --input  data/internal/deepseek_v3_671b_8k/raw/<your_file>.csv \
    --output data/internal/deepseek_v3_671b_8k/requests.jsonl
```

**脚本特性**：
- 流式处理，单行读写，安全处理 1 GB+ 文件
- 每 10,000 行打印进度
- 支持 ISO-8601 和 Unix 浮点两种时间戳格式
- 解析失败的行打印警告并写空 `raw_prompt`，不中断

**可选参数**：

```bash
# CSV 有表头行
python scripts/convert_csv_to_jsonl.py --input ... --output ... --has-header

# 列顺序不同（如 tenant_id 在第 0 列）
python scripts/convert_csv_to_jsonl.py --input ... --output ... \
    --col-request-id 1 --col-tenant-id 0 --col-params 2 --col-timestamp 3

# 指定输入文件编码（如 GBK）
python scripts/convert_csv_to_jsonl.py --input ... --output ... --encoding gbk
```
...
此命令依次执行 10 项分析，输出全部写入 `outputs/maas/deepseek_v3_671b_8k/`：

| 步骤 | 分析内容 | 输出目录 |
|---|---|---|
| 1 | F4 前缀命中率趋势（等价 vLLM APC 命中率）| `f4_prefix/` |
| 2 | F4 最宽口径复用率趋势 | `f4_reusable/` |
| 3 | F13 前缀命中 reuse_time CDF | `f13_prefix/` |
| 4 | F13 最宽口径 reuse_time CDF | `f13_reusable/` |
| 5 | reuse_rank Pareto 分布 | `reuse_rank/` |
| 6 | top_ngrams 高频 block 序列 | `top_ngrams/` |
| 7 | E1 per-user 前缀命中率（4 个 block_size）| `e1_user_hit_rate/` |
| 8 | E1-B 命中贡献 Lorenz 曲线 | `e1b_skewness/` |
| 9 | E5 高频序列文本还原 | `e5_block_text/` |
| 10 | Benchmark：TrieIndex vs RadixTrieIndex | `benchmark_index/` |

也可单独运行某一项（直接调用对应脚本 + 对应 YAML）：

```bash
# 只运行 F4 前缀命中分析
python scripts/generate_f4_business.py configs/maas/deepseek_v3_671b_8k/f4_prefix.yaml

# 只运行 E5 文本还原
python scripts/generate_e5_block_text.py configs/maas/deepseek_v3_671b_8k/e5_block_text.yaml
```
...
每个模型走完全相同的流程，只需替换 `MODEL_NAME`：

```bash
# 为每个模型创建目录和配置（仿照 deepseek_v3_671b_8k 目录结构）
# 1. 复制配置目录
cp -r configs/maas/deepseek_v3_671b_8k configs/maas/<new_model_name>

# 2. 批量修改 trace_name 和 input_file（sed 替换）
sed -i 's/deepseek_v3_671b_8k/<new_model_name>/g' configs/maas/<new_model_name>/*.yaml

# 3. 放置 CSV 并转换
python scripts/convert_csv_to_jsonl.py \
    --input  data/internal/<new_model_name>/raw/<file>.csv \
    --output data/internal/<new_model_name>/requests.jsonl

# 4. 运行分析
bash scripts/run_maas_analysis.sh <new_model_name>
```

**跨模型对比注意事项**：
- 每个模型的分析结果完全独立，不要跨模型合并 JSONL
- 对比不同模型的命中率时，需确认所用 `block_size` 一致
- `e5_block_text` 输出的高频文本内容在不同模型之间可以对比，以判断是否存在共同的高复用 prompt 模板
...
data/internal/
  deepseek_v3_671b_8k/
    raw/                    ← 放原始 CSV
    requests.jsonl          ← 转换后的 JSONL（自动生成）

configs/maas/
  deepseek_v3_671b_8k/     ← 10 个 YAML 配置文件（已就绪）
    f4_prefix.yaml
    f4_reusable.yaml
    f13_prefix.yaml
    f13_reusable.yaml
...
outputs/maas/
  deepseek_v3_671b_8k/     ← 所有分析输出（自动创建）
    f4_prefix/
    f4_reusable/
    f13_prefix/
    f13_reusable/
