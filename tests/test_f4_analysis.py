...
Coverage matrix
---------------
1.  reusable version uses content_reused_blocks_anywhere field
2.  prefix version uses content_prefix_reuse_blocks field
...
def _make_results() -> list[PerRequestResult]:
    return [
        PerRequestResult("req1", 0.0,   0, total_blocks=3, content_prefix_reuse_blocks=0, content_reused_blocks_anywhere=0),
        PerRequestResult("req2", 60.0,  1, total_blocks=3, content_prefix_reuse_blocks=2, content_reused_blocks_anywhere=3),
        PerRequestResult("req3", 120.0, 2, total_blocks=4, content_prefix_reuse_blocks=1, content_reused_blocks_anywhere=2),
    ]
...
def _make_results_with_empty() -> list[PerRequestResult]:
    ...
    return [
        PerRequestResult("req1",  0.0, 0, total_blocks=3, content_prefix_reuse_blocks=0, content_reused_blocks_anywhere=0),
        PerRequestResult("empty", 30.0, 1, total_blocks=0, content_prefix_reuse_blocks=0, content_reused_blocks_anywhere=0),
        PerRequestResult("req2",  60.0, 2, total_blocks=3, content_prefix_reuse_blocks=2, content_reused_blocks_anywhere=3),
    ]


# ---------------------------------------------------------------------------
# 1. reusable version uses content_reused_blocks_anywhere
# ---------------------------------------------------------------------------

def test_reusable_hit_definition_field_name() -> None:
    series = compute_f4_series(_make_results(), hit_metric="content_block_reuse", bin_size_seconds=60)
    assert series.hit_definition == "content_reused_blocks_anywhere"
...
# ---------------------------------------------------------------------------
# 2. prefix version uses content_prefix_reuse_blocks
# ---------------------------------------------------------------------------

def test_prefix_hit_definition_field_name() -> None:
    series = compute_f4_series(_make_results(), hit_metric="content_prefix_reuse", bin_size_seconds=60)
    assert series.hit_definition == "content_prefix_reuse_blocks"


def test_content_prefix_reuse_blocks_sum() -> None:
    series = compute_f4_series(_make_results(), hit_metric="content_prefix_reuse", bin_size_seconds=60)
...
def test_total_blocks_sum_identical_across_metrics() -> None:
    s_r = compute_f4_series(_make_results(), hit_metric="content_block_reuse", bin_size_seconds=60)
    s_p = compute_f4_series(_make_results(), hit_metric="content_prefix_reuse",   bin_size_seconds=60)
...
def test_overall_ratio_invariant_for_both_metrics() -> None:
    for metric in ("content_block_reuse", "content_prefix_reuse"):
...
def test_empty_records_excluded_from_total() -> None:
    ...
    series = compute_f4_series(_make_results_with_empty(), hit_metric="content_block_reuse", bin_size_seconds=60)
...
def test_metadata_hit_definition_recorded_for_reusable(tmp_path: Path) -> None:
    ...
    meta = json.loads(p.read_text())
    assert meta["hit_definition"] == "content_reused_blocks_anywhere"
...
def test_metadata_hit_definition_recorded_for_prefix(tmp_path: Path) -> None:
    ...
    save_metadata_json(series, p, trace_name="traceA", input_file="test.jsonl")
    meta = json.loads(p.read_text())
    assert meta["hit_definition"] == "content_prefix_reuse_blocks"
...
def test_two_variant_output_dirs_are_independent(tmp_path: Path) -> None:
    ...
    dirs = {
        "content_block_reuse": tmp_path / "f4_traceA_public_reusable",
        "content_prefix_reuse":   tmp_path / "f4_traceA_public_prefix",
    }
    ...
    assert (dirs["content_block_reuse"] / "series.csv").exists()
    assert (dirs["content_prefix_reuse"] / "series.csv").exists()
    assert dirs["content_block_reuse"] != dirs["content_prefix_reuse"]
    # Files with the same name in different dirs are different objects
    r_meta = json.loads((dirs["content_block_reuse"] / "metadata.json").read_text())
    p_meta = json.loads((dirs["content_prefix_reuse"]   / "metadata.json").read_text())
...
def test_empty_results_returns_zero_series() -> None:
    series = compute_f4_series([], hit_metric="content_block_reuse", bin_size_seconds=60)
...
def test_all_empty_block_records_gives_zero_totals() -> None:
    results = [
        PerRequestResult("a", 0.0, 0, total_blocks=0, content_prefix_reuse_blocks=0, content_reused_blocks_anywhere=0),
        PerRequestResult("b", 60.0, 1, total_blocks=0, content_prefix_reuse_blocks=0, content_reused_blocks_anywhere=0),
    ]
    series = compute_f4_series(results, hit_metric="content_block_reuse", bin_size_seconds=60)
    assert series.total_blocks_sum == 0
    assert series.ideal_overall_hit_ratio == 0.0
...
def test_single_bin_aggregation() -> None:
    ...
    results = [
        PerRequestResult("r1", 0.0,  0, total_blocks=4, content_prefix_reuse_blocks=0, content_reused_blocks_anywhere=0),
        PerRequestResult("r2", 30.0, 1, total_blocks=4, content_prefix_reuse_blocks=3, content_reused_blocks_anywhere=4),
    ]
    series = compute_f4_series(results, hit_metric="content_prefix_reuse", bin_size_seconds=60)
...
def test_bin_start_hours_conversion() -> None:
    series = compute_f4_series(_make_results(), hit_metric="content_block_reuse", bin_size_seconds=3600)
