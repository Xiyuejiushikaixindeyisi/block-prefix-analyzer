...
def _simple_series(hit_metric: str = "reusable"):
    results = [
        PerRequestResult("r1", 0.0,   0, total_blocks=3, prefix_hit_blocks=0, reusable_block_count=0),
        PerRequestResult("r2", 60.0,  1, total_blocks=3, prefix_hit_blocks=2, reusable_block_count=3),
        PerRequestResult("r3", 120.0, 2, total_blocks=4, prefix_hit_blocks=1, reusable_block_count=2),
    ]
...
def test_plot_f4_creates_png_file(tmp_path: Path) -> None:
    series = _simple_series("reusable")
...
def test_plot_f4_two_variants_produce_separate_files(tmp_path: Path) -> None:
    ...
    out_r = tmp_path / "reusable" / "plot.png"
    out_p = tmp_path / "prefix" / "plot.png"
    plot_f4(_simple_series("reusable"), out_r, title="reusable")
    plot_f4(_simple_series("prefix"),   out_p, title="prefix")
...
def test_plot_f4_empty_series_does_not_raise(tmp_path: Path) -> None:
    from block_prefix_analyzer.analysis.f4 import compute_f4_series
    series = compute_f4_series([], hit_metric="reusable")
